...
do
    ...
    echo "----------------------------------"
    grep "Signature Algorithm" "$filename.txt" | sed -n '2,2p'
...
do
    ...
    echo "-----------------------------------------------------------------------------------------"
    grep "Version" "$filename.crl.txt"

    #CRL Number
    echo "----------"
    echo "CRL Number"
    echo "----------"
    grep -A 1 "CRL Number" "$filename.crl.txt"

    #Signature
    echo "----------------------------------------------"
    echo "Signature: Vérifier l'algorithme de signature."
    echo "----------------------------------------------"
    grep "Signature Algorithm" "$filename.crl.txt" | sed -n '2,2p'

    #Issuer
    echo "------"
    echo "Issuer"
    echo "------"
    grep "Issuer:" "$filename.crl.txt"

    #Authority Key Identifier
    echo "------------------------"
    echo "Authority Key Identifier"
    echo "------------------------"
    grep -A 1 "Authority Key Identifier" "$filename.crl.txt"

    #Update
    echo "----------------------------------------------------------------------------------------------------------------------------"
    echo "This Update"
    echo "Next Update. Il est recommandé que ce champ soit fonction de la fréquence de publication définie dans le document [RGS_A_13]"
    echo "----------------------------------------------------------------------------------------------------------------------------"
    grep "Update" "$filename.crl.txt"
    ...
    echo "Les raisons de révocation ne doivent pas être publiées (à vérifier)"
    echo "-------------------------------------------------------------------------"
    grep -A 2 "CRL entry extensions:" "$filename.crl.txt"
